...
```text
https://github.com/ActionFit-Editor/UI_Foundation.git#2.0.5
```
...
UGUI와 Localization은 `package.json`의 hard dependency이므로 설치 시 함께 해석되어야 합니다. Localization을 사용하지 않는 소비 프로젝트라도 2.0.5에서는 의존성을 제거할 수 없습니다.
...
2.0.5는 기존 냥카페 prefab/scene 호환을 위해 다음을 의도적으로 유지합니다.
...
패키지를 적용한 뒤 prefab/scene에 `Missing Script`가 없는지 확인하고, 필요하면 **Tools > Package > UI Foundation > Migrate Component Refs**를 실행해 `_image`, `_inputField`, `_scrollRect` 캐시를 채우세요.
...
기존 `AddListener`, `RemoveListener`, `RemoveAllListeners`, `SetDisable`, `SetEnable`, `SetInteractable`, `IsDisabled` 사용 방식은 유지됩니다. press scale 설정도 `UI_Button` Inspector에 통합됩니다. 독립 `UIButtonPressEffect` 타입과 GUID는 `UI_Button`이 아닌 일반 UGUI 대상의 호환을 위해 남아 있습니다. 공개 `UI_Button.Button` 접근자는 제거되었으며 키보드/Gamepad submit과 UGUI Navigation은 이 포인터 전용 계약에 포함되지 않습니다.

`Use Disable Text Color`는 `SetDisable()`이 실행될 때 각 TMP의 현재 `fontSharedMaterial`을 기준으로 어두운 재질을 적용합니다. `SetEnable()`과 `OnDisable()`은 버튼이 실제로 교체했고 아직 그대로 남아 있는 재질만 원본 참조로 복원하므로, `UI_Text`가 런타임에 적용한 outline 재질이나 이후 다른 소유자가 교체한 재질을 덮어쓰지 않습니다. 반복 호출과 `targetTexts`의 null 항목은 안전하게 무시됩니다.
...
`UI_Text.SetLocalizeKey(table, entry)`는 즉시 텍스트를 적용하고 locale 변경 갱신 대상으로 등록합니다. 스마트 스트링이나 `string.Format` 인자를 런타임에 바꿀 때는 `SetLocalizeArguments(params object[])`를 호출하면 인자를 보관하고 유효한 키의 현재 locale 텍스트를 즉시 다시 적용합니다. 이 API는 localization 토글과 키를 바꾸지 않으므로 Inspector 설정 또는 `SetLocalizeKey`와 함께 사용하세요.

`UI_Text` 밖에서 locale-dependent text를 관리하는 객체는 `ILocaleRefreshable.RefreshLocalization()`을 구현하고 초기화 시 `UILocalizationRefreshHub.Register(this)`를 한 번 호출할 수 있습니다.
...
`DOTWEEN`을 정의하려면 소비 프로젝트에 호환되는 DOTween core가 설치되어 있고, 그 precompiled assembly가 `com.actionfit.ui.foundation` asmdef에서 auto-reference 가능한 상태여야 합니다. DOTween을 별도 asmdef 소스로 구성했다면 현재 2.0.5 asmdef에는 해당 참조가 없으므로 심벌만 추가해서는 안 됩니다.
...
`ScalePulse`를 인디케이터 GameObject에 붙이면 별도 target 없이 자신의 `Transform`을 공용 펄스에 등록합니다. 최초 활성 인스턴스는 현재 `localScale`의 `0.8` 배율에서 시작하고 0.4초 동안 최대 배율까지 커졌다가 다시 작아집니다. 이후 활성화된 인스턴스는 기존 위상을 재시작하지 않고 현재 공통 위상에 합류합니다.

등록자가 한 개 이상일 때만 하나의 `Awaitable.NextFrameAsync` 루프가 실행됩니다. 컴포넌트별 `Update`는 없으며 마지막 인스턴스가 비활성화되면 루프가 정지하고 각 Transform은 활성화 시 캡처한 정확한 기준 스케일로 복원됩니다. `Time.deltaTime`을 사용하므로 `Time.timeScale == 0`에서는 위상 진행이 멈춥니다.

기존 Animator 자산은 `ScalePulseAnimatorMigrator`의 명시적 request로 전환할 수 있습니다. Preview는 exact Controller GUID, prefab/scene 경로, 중첩 프리팹 component 참조를 읽기만 하고, Apply는 일치하는 Animator YAML 블록을 동일 component fileID의 `ScalePulse`로 교체하며 명시된 stripped Animator 참조도 MonoBehaviour로 전환합니다. Controller GUID와 대상·제외 경로·중첩 참조는 소비 프로젝트가 소유하며 Foundation은 프로젝트 전체를 자동 검색하거나 저장하지 않습니다. Cat Merge Cafe에서는 `Tools > Project > Indicator ScalePulse > Preview`를 먼저 실행하고 `Apply...`에서 표시된 비-Lava-Rush 대상만 저장합니다.
...
새 `UI_Text`의 Inspector 기본값은 Outline Width `0.1`, Underlay Offset Y `-0.5`입니다. 토글이 꺼져 있으면 해당 값은 렌더링에 적용되지 않습니다.
...
- Cat Merge Cafe 전용 **Tools > Project > Indicator ScalePulse > Preview / Apply...**는 프로젝트 adapter가 정확한 Controller GUID와 자산 경로를 Foundation migrator에 전달합니다.
...
2.0.5는 `Tests/Runtime`의 `com.actionfit.ui.foundation.Runtime.Tests`와 `Tests/Editor`의 `com.actionfit.ui.foundation.Editor.Tests`를 포함합니다. 두 어셈블리 모두 `autoReferenced: false`, `UNITY_INCLUDE_TESTS` 조건이며 Unity Test Framework의 `TestAssemblies` 참조를 사용합니다. **Window > General > Test Runner**에서 실행하세요.
...
- `ScalePulseTests`와 `ScalePulseAnimatorMigratorTests`: 빈 레지스트리 무동작, 공통 위상 합류, 기준 스케일 복원, exact GUID/fileID 보존, 제외 경로와 재적용 멱등성
- `UIWrapperBehaviorTests`: `UI_Button`이 실제로 소유한 비활성 TMP 재질만 복원하고 런타임 outline 및 외부 재질 교체를 보존하는 회귀
