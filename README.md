...
- 패키지 ID: `com.actionfit.ui.foundation`
- 버전: `2.0.1`
- 최소 Unity 버전: `6000.2`
- Public 저장소: <https://github.com/ActionFit-Editor/UI_Foundation>
...
{
  "dependencies": {
    "com.actionfit.ui.foundation": "https://github.com/ActionFit-Editor/UI_Foundation.git#2.0.1"
  }
}
```
...
```text
https://github.com/ActionFit-Editor/UI_Foundation.git#2.0.1
```
...
UGUI와 Localization은 `package.json`의 hard dependency이므로 설치 시 함께 해석되어야 합니다. Localization을 사용하지 않는 소비 프로젝트라도 2.0.1에서는 의존성을 제거할 수 없습니다.
...
- 버튼: Image 기반 `UI_Button`, Image 없는 `UI_Clickable`, 활성/비활성 비주얼, 클릭음/햅틱/상태색/테마 provider, press/enable 애니메이션
...
2.0.1은 기존 냥카페 prefab/scene 호환을 위해 다음을 의도적으로 유지합니다.
...
패키지를 적용한 뒤 prefab/scene에 `Missing Script`가 없는지 확인하고, 필요하면 **Tools > Package > UI Foundation > Migrate Component Refs**를 실행해 `_image`, `_inputField`, `_scrollRect` 캐시를 채우세요. 이 메뉴와 버튼 마이그레이터는 소비 프로젝트의 `Assets/`만 검색·저장하며 설치 package asset은 수정하지 않습니다.

## `UI_Button` 입력 계약과 2.0 마이그레이션

`UI_Button`은 더 이상 같은 GameObject의 `UnityEngine.UI.Button`이나 `UIButtonPressEffect`에 의존하지 않습니다. 자체 `Interactable`과 `On Click ()` 직렬화 필드를 소유하고 `IPointerEnterHandler`, `IPointerExitHandler`, `IPointerDownHandler`, `IPointerUpHandler`, `IPointerClickHandler`를 직접 처리합니다. 왼쪽 포인터, 활성/enable 상태, 상위 `CanvasGroup`의 `interactable`·`blocksRaycasts`·`ignoreParentGroups`를 확인한 뒤 클릭 이벤트와 클릭음을 실행합니다.

기존 `AddListener`, `RemoveListener`, `RemoveAllListeners`, `SetDisable`, `SetEnable`, `SetInteractable`, `Interactable`, `IsDisabled` 사용 방식과 `SetEvent`, 우클릭, down/up, hold, `SetActive`, `TabEffect`, `ContextText` 호환 API를 제공합니다. press scale 설정도 `UI_Button` Inspector에 통합됩니다. `Transition=false`인 파생 타입은 press scale을 적용하지 않습니다. 독립 `UIButtonPressEffect` 타입과 GUID는 `UI_Button`이 아닌 일반 UGUI 대상의 호환을 위해 남아 있습니다. native `Button`을 노출하는 `ComponentButton`은 제공하지 않으며 키보드/Gamepad submit과 UGUI Navigation은 이 포인터 전용 계약에 포함되지 않습니다.

`UI_Button : UI_Image`와 `[RequireComponent(typeof(Image))]` 계약은 기존 직렬화/API 호환을 위해 유지합니다. Graphic을 소유하지 않는 클릭 영역에는 `UI_Clickable : UI_Rect`를 사용하세요. `UI_Clickable`은 native `Image`, `Button`, `EventTrigger`를 자동 추가하지 않으며, 실제 raycast는 같은 오브젝트나 자식의 프로젝트 소유 `Graphic`이 담당합니다. 두 타입을 기계적으로 일괄 교체하지 말고 Image 유무와 호출 API를 먼저 분류해야 합니다.

`UI_Image`는 레거시 전환을 위한 one-shot virtual `Initialize`, `Material`, `Rect`, `Fill`, `SetColor`, `SetFill` 별칭과 editor-only Sprite Swap marker key를 유지합니다. `UI_Scroll`은 기존 API와 함께 `Initialize`, `Enabled`, layout refresh, 상하좌우/자식 scroll, `StopScroll` 호환 API를 제공합니다.

`UI_Text`는 기존 fluent API와 함께 one-shot `Initialize()` 및 파생 타입용 `OnEnable`/`OnDisable` 수명주기 확장 지점을 제공합니다. 레거시 버튼의 `ContextText`를 유지하려면 자식 `UI_Text`를 부모 `UI_Button`보다 먼저 이관하세요.

Editor에서 새 `UI_Image` 또는 `UI_Button`을 추가하거나 Reset할 때 native `Image.sprite`가 `None`이고 기본 Material을 사용 중이면 Unity 기본 `UI/Skin/UIMask.psd`를 설정합니다. 이미 다른 Sprite가 있거나 custom Material이 렌더 텍스처를 공급하면 덮어쓰지 않으며, 기존 prefab/scene을 `OnValidate`로 일괄 변경하지 않습니다.

기존 자산은 먼저 **Preview Legacy UI_Button Migration**으로 검사한 뒤 **Apply Legacy UI_Button Migration...**을 명시적으로 실행하세요. 마이그레이터는 같은 GameObject의 native `Button.interactable`, persistent `m_OnClick`, `UIButtonPressEffect`의 enable/scale/target 설정을 새 필드로 복사한 뒤 해당 두 legacy component만 제거합니다. prefab 상속과 scene override를 보존하며 재실행해도 추가 변경이 없어야 합니다.
...
- `IUIButtonHapticPlayer`: 버튼 클릭 햅틱을 프로젝트 피드백 시스템으로 전달
- `IUIButtonStateColorTheme`: `SetActive`의 활성/비활성 색상을 프로젝트에서 제공
- `IUIButtonTheme`: `UI_Button.ButtonSprite` preset을 프로젝트 Sprite로 변환
- `UIButtonServices`: 위 provider의 런타임 등록 지점
...
provider를 등록하지 않아도 패키지는 동작합니다. 클릭음과 햅틱은 재생하지 않고, theme lookup은 `null`, 상태색은 white/기본 disabled gray로 fallback합니다. `UIButtonServices`는 `SubsystemRegistration`에서 초기화되므로 런타임 provider는 그 이후 단계(예: `BeforeSceneLoad`)에 다시 등록해야 합니다.
...
`DOTWEEN`을 정의하려면 소비 프로젝트에 호환되는 DOTween core가 설치되어 있고, 그 precompiled assembly가 `com.actionfit.ui.foundation` asmdef에서 auto-reference 가능한 상태여야 합니다. DOTween을 별도 asmdef 소스로 구성했다면 현재 2.0.1 asmdef에는 해당 참조가 없으므로 심벌만 추가해서는 안 됩니다.
...
| Glyph Metrics | `W`, `H` | glyph 너비/높이 |
| Glyph Metrics | `BX`, `BY` | horizontal bearing X/Y 값 |
| Glyph Metrics | `AD` | horizontal advance 값 |
...
- **Tools > Package > UI Foundation > Migrate Component Refs**: 프로젝트 `Assets/`의 prefab과 scene을 순회해 wrapper의 직렬화 component cache를 채우고 변경 자산을 저장합니다.
- **Tools > Package > UI Foundation > Preview Legacy UI_Button Migration**: 저장하지 않고 legacy `Button`/`UIButtonPressEffect` 대상, 경고, 실패를 보고합니다.
- **Tools > Package > UI Foundation > Apply Legacy UI_Button Migration...**: preview와 사용자 확인 후 대상 prefab/scene을 저장하고 잔여 후보가 없는지 다시 검증합니다.

Migration 메뉴는 다수 자산을 검사하거나 수정하고 scene을 차례로 열기 때문에 실행 전에 작업을 commit하거나 백업하고, apply 후 diff를 반드시 검토하세요. 배치 apply는 `UI_ButtonLegacyMigrator.ApplyBatch`와 `-uiButtonMigrationApply` 플래그를 함께 지정해야 저장합니다.
...
2.0.1은 `Tests/Runtime`의 `com.actionfit.ui.foundation.Runtime.Tests`와 `Tests/Editor`의 `com.actionfit.ui.foundation.Editor.Tests`를 포함합니다. 두 어셈블리 모두 `autoReferenced: false`, `UNITY_INCLUDE_TESTS` 조건이며 Unity Test Framework의 `TestAssemblies` 참조를 사용합니다. **Window > General > Test Runner**에서 실행하세요.
...
- `UI_ButtonLegacyMigratorTests`: persistent click, interactable, press 설정, prefab variant/scene override, idempotence와 Missing Script 회귀
- `UI_ButtonLegacyCompatibilityTests`, `UIClickableBehaviorTests`, `UIScrollLegacyCompatibilityTests`: 레거시 호출 API, feedback/state, image-less 클릭, CanvasGroup, scroll endpoint/layout 계약
...
- 포인터 enter/exit/down/up/click, `CanvasGroup` 차단, press 취소와 legacy migration preview/apply 결과
